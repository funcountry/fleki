...
def _forbid_keys(data: Mapping[str, Any], keys: set[str], context: str) -> None:
    ...
    if present:
        raise ValidationError(
            f"{context} must not include repository-owned render keys: {', '.join(present)}"
        )
...
def validate_save_decision(
    ...
    _forbid_keys(
        decision,
        {"pdf_render", "render_bundle", "render_manifest"},
        "save decision",
    )
    _require_keys(
        decision,
    ...
    for action in decision["topic_actions"]:
        ...
        if lifecycle_state is not None and lifecycle_state not in SAVE_LIFECYCLE_STATES:
            raise ValidationError(
                "invalid topic_action.lifecycle_state; save only accepts current or historical"
            )
        for unit in action["knowledge_units"]:
            ...
            if temporal_scope is not None and temporal_scope not in TEMPORAL_SCOPES:
                raise ValidationError("invalid knowledge_unit.temporal_scope")
